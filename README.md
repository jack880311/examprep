...
**重要安全提示：**
- ❌ **API Key 完全不會硬碼在任何 HTML / 程式碼中**
- ✅ **首次使用時系統會提示你輸入 API Key**
- ✅ **API Key 安全儲存在你的本機瀏覽器（localStorage）**
...
- 該服務的 API Key（在首次使用時輸入，只需輸入一次）

**如何修改 API 位址和模型：**

編輯 `index.html` 第 1091-1093 行：

```javascript
let AI_ENDPOINT = 'http://100.115.60.107:4142/v1/chat/completions';  // 改成你的 API 位址
let AI_MODEL = 'gemini-3-flash-preview';  // 改成你的模型名稱
let AI_KEY = localStorage.getItem('gcp_ai_key') || '';  // ← 不要改這行！使用者會在使用時輸入
```

然後執行：
```bash
python3 generate_embedded.py    # 重新生成嵌入式版本
python3 generate_mobile.py      # 重新生成行動版本
```

修改後，GitHub Pages 和本地版本都會自動使用新的 API 設定。
...
> 🔐 **安全提示**：首次使用 AI 功能時，系統會提示你輸入 API Key。API Key 會**安全儲存在你的本機瀏覽器**，不會上傳到 GitHub。
