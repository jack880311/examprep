...
| ⭐ **書籤 / 標記** | 標記重要或易錯題目 |
| 📝 **每題筆記** | 記錄答題思路，包含於進度備份 |
| 🔄 **重考模式** | 點「🔄 重考」按鈕，勾選標記題 / 答錯題，隱藏答案重新作答 |
...
**筆記**：每題展開「📝 我的筆記」，輸入後 500ms 自動儲存，包含於進度 JSON 備份。設計用途是在**考試或練習當下**記錄答題思路——例如「為什麼選 C 不選 B」「這個概念還不確定，要再查」——日後重考時回頭看筆記，就能還原當初為何打書籤的原因。

**重考模式**：點控制列的「🔄 重考」按鈕，在 Modal 中勾選題目來源（⭐ 已標記題 / ❌ 答錯題，可複選），按「開始重考」進入重考模式——答案自動隱藏，題目照原始題號排序，點「顯示答案」只在當次 session 揭曉，**不修改已儲存進度**。
